...
class JInstallerXmap_Ext extends JAdapterInstance
{
	...
	public function install()
	{
		...
		$extension_files = &$this->manifest->getElementByPath('files');
		if ($extension_files INSTANCEOF JSimpleXMLElement && count($extension_files->children()))
		{
			$files = &$extension_files->children();
			foreach ($files as $file)
			{
				if ($file->attributes('xmap_ext'))
				{
					$element = $file->attributes('xmap_ext');
					break;
				}
			}
		}
		...
		if (is_a($this->scriptElement, 'JSimpleXMLElement'))
		{
			...
			// Set the class name
			$classname = 'xmapExt'.$element.'InstallerScript';
		...
		if (!file_exists($this->parent->getPath('extension_root')))
		{
			...
			{
				$this->parent->abort(JText::_('Xmap Extension').' '.JText::_($this->route).': '.JText::_('Failed to create directory').': "'.$this->parent->getPath('extension_root').'"');
				return false;
			}
		}
		...
		if ($this->parent->parseFiles($extension_files, -1) === false)
		{
			// Install failed, roll back changes
			$this->parent->abort();
			return false;
		...
		// Parse optional tags -- media and language files for extensions go in admin app
		...
		if ($this->get('manifest_script'))
		{
			...
			if (!file_exists($path['dest']))
			{
				if (!$this->parent->copyFiles(array ($path)))
				{
					...
					$this->parent->abort(JText::_('Xmap Extension').' '.JText::_($this->route).': '.JText::_('Could not copy PHP manifest file.'));
					return false;
				}
			}
		}
		...
		if ($id)
		{
			...
			{
				// Install failed, roll back changes
				$this->parent->abort(JText::_('Xmap Extension').' '.JText::_($this->route).': '.JText::_('Xmap Extension').' "'. $this->get('name') .'" '.JText::_('already exists!'));
				return false;
			}
		...
		else
		{
			...
			{
				// Install failed, roll back changes
				$this->parent->abort(JText::_('Xmap Extension').' '.JText::_($this->route).': '.$db->stderr(true));
				return false;
			}

			// Since we have created a extension item, we add it to the installation step stack
			// so that if we have to rollback the changes we can undo it.
			$this->parent->pushStep(array ('type' => 'xmap_ext', 'id' => $row->extension_id));
			$id = $row->extension_id;
		}
		...
		{
			// Install failed, rollback changes
			$this->parent->abort(JText::_('Xmap Extension').' '.JText::_($this->route).': '.JText::_('SQLERRORORFILE')." ".$db->stderr(true));
			return false;
		}
		...
		{
			// Install failed, rollback changes
			$this->parent->abort(JText::_('Xmap Extension').' '.JText::_($this->route).': '.JText::_('Could not copy setup file'));
			return false;
		}
	...
	public function uninstall($id)
	{
		...
		if ($row->protected)
		{
			JError::raiseWarning(100, JText::_('Xmap Extension').' '.JText::_('Uninstall').': '.JText::sprintf('WARNCOREPLUGIN', $row->name)."<br />".JText::_('WARNCOREPLUGIN2'));
			return false;
		}

		// Get the extension folder so we can properly build the extension path
		if (trim($row->folder) == '')
		{
			JError::raiseWarning(100, JText::_('Extension').' '.JText::_('Uninstall').': '.JText::_('Folder field empty, cannot remove files'));
			return false;
		}

		// Set the extension root path
		$this->parent->setPath('extension_root', JPATH_ROOT.DS.'extensions'.DS.$row->folder.DS.$row->element);
		...
		if (file_exists($manifestFile))
		{
			...
			if (is_a($this->scriptElement, 'JSimpleXMLElement'))
			{
				...
				// Set the class name
				$classname = 'extension'.$row->folder.$row->element.'InstallerScript';
				if (class_exists($classname))
				{
					// create a new instance
					$this->parent->manifestClass = new $classname($this);
	...
	{
		$results = Array();
		$folder_list = JFolder::folders(JPATH_SITE.DS.'extensions');

		foreach ($folder_list as $folder)
		{
			$file_list = JFolder::files(JPATH_SITE.DS.'extensions'.DS.$folder,'\.xml$');
			foreach ($file_list as $file)
			{
				$file = JFile::stripExt($file);
				if ($file == 'example') continue; // ignore example extensions
			...
			$folder_list = JFolder::folders(JPATH_SITE.DS.'extensions'.DS.$folder);
			foreach ($folder_list as $extension_folder)
			{
				$file_list = JFolder::files(JPATH_SITE.DS.'extensions'.DS.$folder.DS.$extension_folder,'\.xml$');
				foreach ($file_list as $file)
				{
					$file = JFile::stripExt($file);
					if ($file == 'example') continue; // ignore example extensions
	...
	{
		// Extensions use the extensions table as their primary store
		// Similar to modules and templates, rather easy
		// If its not in the extensions table we just add it
		$client = JApplicationHelper::getClientInfo($this->parent->extension->client_id);
		if (is_dir($client->path . DS . 'extensions'. DS . $this->parent->extension->folder . DS . $this->parent->extension->element)) {
			$manifestPath = $client->path . DS . 'extensions'. DS . $this->parent->extension->folder . DS . $this->parent->extension->element . DS . $this->parent->extension->element . '.xml';
		}
		else {
			$manifestPath = $client->path . DS . 'extensions'. DS . $this->parent->extension->folder . DS . $this->parent->extension->element . '.xml';
		}
		...
		{
			JError::raiseWarning(101, JText::_('Extension').' '.JText::_('Discover Install').': '.JText::_('Failed to store extension details'));
			return false;
		}
	}
	...
	function refreshManifestCache()
	{
		...
		{
			JError::raiseWarning(101, JText::_('Extension').' '.JText::_('Refresh Manifest Cache').': '.JText::_('Failed to store extension details'));
			return false;
		}
	}
}
